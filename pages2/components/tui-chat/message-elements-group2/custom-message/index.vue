<template>
	<view>
			...
			<view class="custom-content">
				...
				<view class="custom-content-score">
					<image v-for="(item, index) in renderDom[0].score" :key="index" class="score-star"
						src="/pages2/static/static/images/star.png"></image>
				</view>
				<view class="custom-content-description">{{ renderDom[0].description }}</view>
			</view>
		</view>
		...
		<view v-if="renderDom[0].type === 'CustomIllnessMessage'"
		...
		<view v-if="renderDom[0].type === 'CustomChuFangMessage'"
			:class="'custom-message custom-message-wrap ' + (isMine ? 'my-custom' : '')">
			<view class="wrap2">
				<view class="top">
				...
				<view class="middle">
					...
					<view class="right">
						<view class="list">
							<view class="item" v-for="item in renderDom[0].cfInfo.medicalOrdersDetails"
								:key="item.drugCode">
								<view class="name">
								...
								<view class="desc">
									{{item.useFrequencyName}}；{{item.useNum}}{{item.useUnit}}/1次；{{item.drugUsemethodName}}
								</view>
							</view>
						</view>
					</view>
		...
		<view v-if="renderDom[0].type === 'CustomAppointmentTimeMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
				...
				<view v-if="renderDom[0].title === '预约时间'" class="custom-content-action"
					@tap="cardTrigger(renderDom[0])">预约时间>></view>
			</view>
		</view>
		<view v-if="renderDom[0].type === 'CustomArticleMessage'"
			:class="'custom-message custom-message-wrap ' + (isMine ? 'my-custom' : '')">
			<view class="wrap">
				<view class="title">
		...
		<!-- //TODO 推送套餐需求 -->
		<view v-if="renderDom[0].type === 'CustomTaoCanMessage'"
			:class="'custom-message custom-message-wrap ' + (isMine ? 'my-custom' : '')">
			<view class="wrap">
				<view class="title">
				...
				<view style="padding: 0 24rpx 30rpx 24rpx;font-size: 28rpx;color: #999;"><text>医生给您推荐了一个服务套餐，请您
						点击购买。</text></view>
		...
		<view v-if="renderDom[0].type === 'CustomWenJuanMessage'"
			:class="'custom-message custom-message-wrap ' + (isMine ? 'my-custom' : '')">
			<view class="wrap">
				<view class="title">
					<image src="/pages2/static/static/images/group/icon_paper.png"></image>{{ renderDom[0].title }}
					<image src="/pages2/static/static/images/group/done.png" class="abs" v-if="renderDom[0].done">
					</image>
				</view>
				<view class="content">
		...
		<view v-if="renderDom[0].type === 'CustomAnalyseMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
				<view class="custom-content-content"><text style="color: #000000;">医生：</text>{{ renderDom[0].doctor }}
				</view>
				<view class="custom-content-content"><text style="color: #000000;">时间：</text>{{ renderDom[0].time }}
				</view>
				<view class="custom-content-action" @tap="modalTrigger(renderDom[0])">查看小结>></view>
			</view>
		</view>
		<view v-if="renderDom[0].type === 'CustomDoctorReceptionMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
				<view class="custom-content-content"><text style="color: #000000;">医生：</text>{{ renderDom[0].doctor }}
		...
		<view v-if="renderDom[0].type === 'CustomDoctorRefuseMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
				<view class="custom-content-content"><text style="color: #000000;">医生：</text>{{ renderDom[0].doctor }}
				</view>
				<view class="custom-content-content"><text style="color: #000000;">原因：</text>{{ renderDom[0].reason }}
				</view>
			</view>
		</view>
		<view v-if="renderDom[0].type === 'CustomDoctorChuFangMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
				<view class="custom-content-content"><text
						style="color: #000000;">诊断结果：</text>{{ renderDom[0].diagnosis }}</view>
				<view class="custom-content-content"><text
						style="color: #000000;">用药建议：</text>{{ renderDom[0].suggestion }}</view>
				<view class="custom-content-action" @tap="previewChufang(renderDom[0])">查看详情>></view>
			</view>
		</view>
		<view v-if="renderDom[0].type === 'FengshikeIllnessMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
		...
		<view v-if="renderDom[0].type === 'Fengshike2IllnessMessage'"
			:class="'custom-message ' + (isMine ? 'my-custom' : '')">
			<view class="custom-content">
				<view class="custom-content-title">{{ renderDom[0].title }}</view>
				<view class="custom-content-content">{{ renderDom[0].content }}</view>
		...
		<view class="wenzhen">
			<u-modal class="wenzhen-modal" title="问诊小结" confirmText="确定" :show="showWenzhen" closeOnClickOverlay
				@confirm="() => showWenzhen = false" @close="() => showWenzhen = false">
				<u--form labelPosition="left" ref="form">
					<u-form-item label="病情描述">
						<view>{{ wenzhen.chiefComplaint }}</view>
					</u-form-item>
					<u-form-item label="处置建议">
						<view>{{ wenzhen.treatment }}</view>
					</u-form-item>
					<u-form-item label="医生">
