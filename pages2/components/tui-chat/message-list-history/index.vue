			...
			<view v-for="(item, index) in messageList" :key="item.ID" class="t-message">
				<view v-if="conversation.type !== '@TIM#SYSTEM'" :id="item.ID">
					<view class="t-message-item">
						<view class="t-message-time" v-if="ifMessageTime(item, index)">
							{{ $u.timeFormat(item.time, 'yyyy-mm-dd hh:MM:ss') }}</view>
						...
						<view v-if="item.type !== 'TIMGroupTipElem'"
							...
							<image class="t-message-avatar" v-if="item.flow === 'in'"
								:src="item.avatar || '/pages2/static/static/images/avatar_21.png'"></image>
		...
		methods: {
			...
			getMessageList(conversation) {
				var params= {
						...
						pageSize: 20,
						sort: 1,
						toAccount: conversation.toUserID+'',
						fromAccount: conversation.userID+''
					}
				uni.$u.http.get('/im-api/tencentIM/queryHistoryIMRecordPage', {
					params:params
				}).then(res => {
					if (res.code === 0 ) {
					            var messageList = res.data.rows; // 消息列表。
								      ...
								      messageList.forEach(function (item, index) {
								            ...
								            item.payload={}
								            item.flow=item.fromAccount===params.fromAccount?'out':'in'
								...
								this.isCompleted =res.data.pageNo>=res.data.totalPage,
