...
export default {
	...
	methods: {
		...
		getMessageList(conversation) {
			...
			var docID = uni.getStorageSync('account').user.userId+''
			var params= {
			...
			uni.$u.http.post('/medical-api/rightsUse/qryHistoryByPage', params).then(res => {
				if (res.code === 0 ) {
							...
							messageList.forEach(function (item, index) {
							      ...
							      item.flow=item.from==docID?'out':'in'
