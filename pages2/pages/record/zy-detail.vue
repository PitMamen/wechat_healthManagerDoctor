<template>
	...
	<view class="page">
		<u-sticky style="top:0">
			<view
				...
				<u-row class="top-h2">
					...
					<u-col span="3">住院天数：</u-col>
					<u-col span="3">{{infodata.zyts}}</u-col>
				</u-row>
				<u-row class="top-h2">
					<u-col span="3">入院科室：</u-col>
					<u-col span="3">{{infodata.ryksmc|| '-'}}</u-col>
					<u-col span="3">出院科室：</u-col>
					<u-col span="3">{{infodata.cyksmc|| '-'}}</u-col>
				</u-row>
				<u-row class="top-h2">
					<u-col span="3">入院时间：</u-col>
					<u-col span="3">{{formatFullDate1(infodata.rysj)}}</u-col>
					<u-col span="3">出院时间：</u-col>
					<u-col span="3">{{formatFullDate1(infodata.cysj)}}</u-col>
				</u-row>
			</view>
		...
		<!-- 诊断信息 -->
		<block v-if="active==1">
			<u-empty v-if=" !infodata.zdxx || infodata.zdxx.length == 0 " mode="data" icon="/pages2/static/img/icon_nodata.png" text="无数据" />
			<view class="case-block2" v-for="(item, index) in infodata.zdxx" :key="index">

				<view class="inspection-title">
					<view class="inspection-title-l">{{item.zdbmmc}}</view>
					<view style="color: #178CFC;">{{item.cyqkbmmc}}</view>
				</view>
				<view class="inspection-content">
					<view>{{item.yzdbzmc}}</view>
					<view>{{ formatFullDate1(item.zdsj)}}</view>
				</view>
		...
		<!-- 用药信息 -->
		<view v-if="active==4">
			<u-empty v-if=" !infodata.yzxx || infodata.yzxx.length == 0 " mode="data" icon="/pages2/static/img/icon_nodata.png" text="无数据" />
			<view class="case-block2" v-for="(item, index) in infodata.yzxx" :key="index" v-if="item.yzlx=='1'">
				<view class="medication-title2">
					<view>{{item.yzmxmc}}</view>
				</view>
				<view class="medication-title2">
				...
				<view class="medication-content">
					...
					<view v-if="item.mcsldw && item.mcsl!=0" style="margin-left: 30rpx;">每次{{item.mcsl}}{{item.mcsldw}}
					</view>
					<view v-if="item.yypd" style="margin-left: 30rpx;">{{item.yypd}}</view>
		...
		<block v-if="active==5">
			...
			<u-empty v-if=" !infodata.ssxx || infodata.ssxx.length == 0 " mode="data" icon="/pages2/static/img/icon_nodata.png" text="无数据" />
			<view class="case-block2" v-for="(item, index) in infodata.ssxx" :key="index">

				<view class="inspection-title">
					<view>{{item.yyssczmc}}</view>
					<view class="operation-time">手术时间：{{formatFullDate1(item.ssqssj)}}</view>
				</view>
				<view class="inspection-content">
					<view>手术级别：{{item.ssjb}}</view>
					<view>切口愈合等级：{{item.qkyhdj}}</view>
				</view>
		...
		<block v-if="active==6">
			<view class="case-block2">
				<view class="case-title">入院情况</view>
				<view class="case-content">{{infodata.ryzztz || '暂无数据'}}</view>
			</view>
			<view class="case-block2">
				<view class="case-title">入院诊断</view>
				<view class="case-content">{{infodata.ryzd || '暂无数据'}}</view>
			</view>
			<view class="case-block2">
				<view class="case-title">诊疗过程</view>
				<view class="case-content">{{infodata.zlgc || '暂无数据'}}</view>
			</view>
			<view class="case-block2">
				<view class="case-title">诊疗结果</view>
				<view class="case-content">{{infodata.zljgmc || '暂无数据'}}</view>
			</view>
			<view class="case-block2">
				<view class="case-title">出院诊断</view>
				<view class="case-content">{{infodata.cyzd || '暂无数据'}}</view>
			</view>
			<view class="case-block2">
				<view class="case-title">出院医嘱</view>
				<view class="case-content">{{infodata.cyyz || '暂无数据'}}</view>
			</view>
		</block>
