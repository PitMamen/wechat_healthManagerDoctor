...
<script>
	export default {
		...
		onLoad: function({
			...
			console.log('---middle onLoad bizToken', bizToken)
			console.log('---middle onLoad miniProgramAppId', miniProgramAppId)
			console.log('---middle onLoad miniProgramPath', miniProgramPath)
			console.log('---middle onLoad miniProgramCallBackUrl', miniProgramCallBackUrl)
		...
		onShow: function() {
			...
			/** 从认证小程序返回 */
			if (options.scene === 1038 && options.referrerInfo.extraData && options.referrerInfo.extraData
				.faceResult) {
				console.log('---app onShow 认证成功')
				uni.$emit('caUpdate', {
					msg: this.miniProgramCallBackUrl
				})
				...
				//   console.log('---app onShow 返回前一页1')
				//    previous.reloadPage(this.miniProgramCallBackUrl.replace('https://realnameverify-test','https://realnameverify-test05'))
				//    uni.navigateBack({
				//      delta: 1,
		 	//    })
				// console.log('---app onShow 返回前一页2')
				//  }
			}
		},
		methods: {
			...
			onJump() {
			 let that = this
				wx.navigateToMiniProgram({
					...
					path: this.miniProgramPath + '?bizToken=' + this.bizToken,
					success: (res) => {

						that.goFaceDone = true
