			...
			<scroll-view class="list" :scroll-y="true" @scrolltolower="scrolltolower" v-else>
				<view class="item" v-for="item in list" :key="item.id">
					...
					<view class="nodate" @click="itemClick(item)" v-else>
						...
						<view class="right">
							<view class="row">
								<view class="name bold">{{ item.order_type_name }}</view>
								<view class="value" :class="{yellow: item.types==='收'}">{{ item.settlement_sum }}</view>
							</view>
							<view class="row">
	...
	export default {
		...
		methods: {
			getList() {
				...
				uni.$u.http.post(`/account-api/accountOrderSettlementMaster/getTradeRecord`, {
					pageNo: this.pageNo,
			...
			itemClick(item) {
				uni.navigateTo({
					url: `/pages3/pages/cash/trade-info?id=${item.id}&types=${item.types}`
				});
			}
		}
	}
